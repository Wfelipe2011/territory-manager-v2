...
describe('Territory Bulk Import (e2e)', () => {
  ...
  it('should import multiple territories successfully', async () => {
    ...
    const response = await request(app.getHttpServer()).post('/v1/territories/bulk').set('Authorization', `Bearer ${token}`).send(payload);
  ...
  it('should isolate data between tenants', async () => {
    ...
    await request(app.getHttpServer()).post('/v1/territories/bulk').set('Authorization', `Bearer ${token1}`).send(payload);
  ...
  it('should restrict access to ADMIN role', async () => {
    ...
    const response = await request(app.getHttpServer()).post('/v1/territories/bulk').set('Authorization', `Bearer ${token}`).send({ rows: [] });
