...
describe('Ghost House Flow (e2e)', () => {
  ...
  it('should create a ghost house when a new address is added to a block', async () => {
    ...
    const response = await request(app.getHttpServer())
      ...
      .send({
        name: 'Block A',
        addresses: [{ street: 'Rua Fantasma', zipCode: '12345678' }],
      });
  ...
  it('should remove ghost house when a real house is approved via report', async () => {
    ...
    const territoryBlockAddressId = (
      await prisma.territory_block_address.findFirst({
    ...
    const reportResponse = await request(app.getHttpServer()).post('/v1/reports').set('Authorization', `Bearer ${adminToken}`).send({
    ...
    const approveResponse = await request(app.getHttpServer()).post(`/v1/reports/approve/${reportId}`).set('Authorization', `Bearer ${adminToken}`).send();
