...
describe('DonationController (e2e)', () => {
  ...
  describe('POST /v1/donations', () => {
    it('deve criar um donativo com sucesso usando dados do PayPal', async () => {
      ...
      // Act
      const response = await request(app.getHttpServer()).post('/v1/donations').set('x-api-key', API_KEY).send(donationData);

      // Assert
      expect(response.status).toBe(201);
      expect(response.body).toHaveProperty('id');
      expect(response.body.tenantId).toBe(1);
      expect(response.body.value).toBe(100.0);
    ...
    it('deve rejeitar requisição sem API Key', async () => {
      ...
      // Act
      const response = await request(app.getHttpServer()).post('/v1/donations').send(donationData);
    ...
    it('deve rejeitar requisição com API Key inválida', async () => {
      ...
      // Act
      const response = await request(app.getHttpServer()).post('/v1/donations').set('x-api-key', 'wrong-api-key').send(donationData);
    ...
    it('deve rejeitar campos obrigatórios ausentes', async () => {
      ...
      // Act
      const response = await request(app.getHttpServer()).post('/v1/donations').set('x-api-key', API_KEY).send(incompleteDonation);
