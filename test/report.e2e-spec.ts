...
describe('Report Flow (e2e)', () => {
  ...
  it('should manage reports (create and list)', async () => {
    ...
    const createReportResponse = await request(app.getHttpServer()).post('/v1/reports').set('Authorization', `Bearer ${publisherToken}`).send({
    ...
    const listReportsResponse = await request(app.getHttpServer()).get('/v1/reports').set('Authorization', `Bearer ${adminToken}`);
