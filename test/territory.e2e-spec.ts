...
describe('Territory Flow (e2e)', () => {
  ...
  it('should create, update and list territories', async () => {
    ...
    // 1. Create
    const createResponse = await request(app.getHttpServer()).post('/v1/territories').set('Authorization', `Bearer ${token}`).send({
      name: 'New Territory',
      typeId: type.id,
    });
    ...
    // 2. Update
    const updateResponse = await request(app.getHttpServer()).put(`/v1/territories/${territoryId}`).set('Authorization', `Bearer ${token}`).send({
      id: territoryId,
    ...
    const listResponse = await request(app.getHttpServer()).get('/v1/territories?round=1').set('Authorization', `Bearer ${token}`);
  ...
  describe('Territory V2', () => {
    it('should list territories (V2)', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/v2/territories?page=1&limit=10&sort=name').set('Authorization', `Bearer ${token}`);
  ...
  describe('Territory V1 Extra', () => {
    it('should get territory types', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/v1/territories/types').set('Authorization', `Bearer ${token}`);
    ...
    it('should return 400 for invalid ID in findById', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/v1/territories/invalid-id').set('Authorization', `Bearer ${token}`);
    ...
    it('should return 404 for non-existent ID in delete', async () => {
      const token = createTestToken({ roles: [Role.ADMIN] });
      const response = await request(app.getHttpServer()).delete('/v1/territories/999999').set('Authorization', `Bearer ${token}`);
    ...
    it('should handle error in update (V1)', async () => {
      ...
      const response = await request(app.getHttpServer()).put('/v1/territories/999999').set('Authorization', `Bearer ${token}`).send({ name: 'New Name' });
    ...
    it('should get territory blocks', async () => {
      ...
      const response = await request(app.getHttpServer()).get(`/v1/territories/${territory.id}/blocks`).set('Authorization', `Bearer ${token}`);
    ...
    it('should fail to get territory without round (V1)', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/v1/territories').set('Authorization', `Bearer ${token}`);
    ...
    it('should get territory by id (Admin)', async () => {
      ...
      const response = await request(app.getHttpServer()).get(`/v1/territories/${territory.id}?round=1`).set('Authorization', `Bearer ${token}`);
    ...
    it('should enforce permissions for DIRIGENTE', async () => {
      ...
      const responseOk = await request(app.getHttpServer()).get(`/v1/territories/${territory1.id}?round=1`).set('Authorization', `Bearer ${tokenAssigned}`);
