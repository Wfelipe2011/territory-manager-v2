...
describe('AuthController (e2e)', () => {
  ...
  describe('/v1/login (POST)', () => {
    it('should login successfully with valid credentials', async () => {
      ...
      // Act
      const response = await request(app.getHttpServer()).post('/v1/login').send({
        email: user.email,
        password,
      });
  ...
  describe.skip('/v1/auth/admin/register (POST)', () => {
    it('should register a new admin successfully', async () => {
      ...
      // Act
      const response = await request(app.getHttpServer()).post('/v1/auth/admin/register').set('Authorization', `Bearer ${adminToken}`).send(payload);
    ...
    it('should return 403 if user is not an admin', async () => {
      ...
      const response = await request(app.getHttpServer()).post('/v1/auth/admin/register').set('Authorization', `Bearer ${token}`).send(payload);
  ...
  describe('/v1/auth/admin/users (GET)', () => {
    it('should list users for the admin tenant', async () => {
      ...
      // Act
      const response = await request(app.getHttpServer()).get('/v1/auth/admin/users').set('Authorization', `Bearer ${adminToken}`);
    ...
    it('should return 403 if user is not an admin', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/v1/auth/admin/users').set('Authorization', `Bearer ${token}`);
  ...
  describe.skip('/v1/auth/public/register (POST)', () => {
    it('should register a new user and tenant successfully', async () => {
      ...
      const response = await request(app.getHttpServer()).post('/v1/auth/public/register').send(payload);
  ...
  describe('Route Protection & Roles', () => {
    ...
    it('should return 403 when user does not have required role', async () => {
      ...
      // Act
      const response = await request(app.getHttpServer()).get('/v1/territories/1/addresses').set('Authorization', `Bearer ${token}`);
