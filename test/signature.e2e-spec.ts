...
describe('Signature Flow (e2e)', () => {
  ...
  it('should manage territory and block signatures', async () => {
    ...
    const getSignatureResponse = await request(app.getHttpServer()).get(`/v1/signature/${territorySignatureKey}`);
  ...
  describe('Validation and Error Cases', () => {
    ...
    it('should return 404 for non-existent territory signature in deleteSignatureTerritory', async () => {
      const response = await request(app.getHttpServer()).delete(`/v1/territories/${territory.id}/signature`).set('Authorization', `Bearer ${adminToken}`);
      expect(response.status).toBe(404);
    });
    ...
    it('should return 400 if block signature already exists', async () => {
      // First generation
      await request(app.getHttpServer()).post(`/v1/territories/${territory.id}/blocks/${block.id}/signature/1`).set('Authorization', `Bearer ${adminToken}`);
    ...
    it('should return 404 if round info is not found in findTokenById', async () => {
      ...
      const response = await request(app.getHttpServer()).get(`/v1/signature/${uniqueId}`);
    ...
    it('should return 404 if signature is not found in findTokenById', async () => {
      const response = await request(app.getHttpServer()).get('/v1/signature/non-existent');
      expect(response.status).toBe(404);
