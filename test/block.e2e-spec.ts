...
describe('Block Flow (e2e)', () => {
  ...
  it('should create, update, list and delete blocks', async () => {
    ...
    const listResponse = await request(app.getHttpServer()).get(`/v2/territories/${territory.id}/blocks`).set('Authorization', `Bearer ${token}`);
    ...
    // 3. Get block details
    const detailsResponse = await request(app.getHttpServer()).get(`/v2/territories/${territory.id}/blocks/${blockId}`).set('Authorization', `Bearer ${token}`);
    ...
    // Verify deletion
    const listAfterDelete = await request(app.getHttpServer()).get(`/v2/territories/${territory.id}/blocks`).set('Authorization', `Bearer ${token}`);
  ...
  it('should handle block with no addresses', async () => {
    ...
    const response = await request(app.getHttpServer()).post(`/v2/territories/${territory.id}/blocks`).set('Authorization', `Bearer ${token}`).send({
    ...
    const removeResponse = await request(app.getHttpServer()).post(`/v2/territories/${territory.id}/blocks`).set('Authorization', `Bearer ${token}`).send({
      id: blockId,
  ...
  it('should return 404 when deleting non-existent block', async () => {
    ...
    const response = await request(app.getHttpServer()).delete(`/v2/territories/${territory.id}/blocks/999`).set('Authorization', `Bearer ${token}`);
  ...
  it('should return 403 when user is not ADMIN', async () => {
    ...
    const response = await request(app.getHttpServer()).get(`/v2/territories/1/blocks`).set('Authorization', `Bearer ${token}`);
