...
describe('House Flow (e2e)', () => {
  ...
  it('should manage houses (list, update status, update details)', async () => {
    ...
    const updateDetailsResponse = await request(app.getHttpServer()).put(`/v1/houses/${house.id}`).set('Authorization', `Bearer ${token}`).send({
      number: '101',
      blockId: block.id,
      streetId: address.id,
      territoryId: territory.id,
  ...
  it('should find, create, update and delete houses (Admin)', async () => {
    ...
    // 1. Create
    const createResponse = await request(app.getHttpServer()).post('/v1/houses').set('Authorization', `Bearer ${token}`).send({
      number: '200',
      blockId: block.id,
      streetId: address.id,
      territoryId: territory.id,
    ...
    // 2. Find by ID
    const findResponse = await request(app.getHttpServer()).get(`/v1/houses/${houseId}`).set('Authorization', `Bearer ${token}`);
    ...
    // 4. Delete
    const deleteResponse = await request(app.getHttpServer()).delete(`/v1/houses/${houseId}`).set('Authorization', `Bearer ${token}`);
  ...
  describe('Validation and Permissions', () => {
    ...
    it('should return 400 for invalid territoryId in getAddressPerTerritoryByIdAndBlockById', async () => {
      const response = await request(app.getHttpServer()).get(`/v1/territories/abc/blocks/${block.id}`).set('Authorization', `Bearer ${token}`);
    ...
    it('should return 400 for invalid blockId in getAddressPerTerritoryByIdAndBlockById', async () => {
      const response = await request(app.getHttpServer()).get(`/v1/territories/${territory.id}/blocks/abc`).set('Authorization', `Bearer ${token}`);
      expect(response.status).toBe(400);
    ...
    it('should return 400 for missing round in getAddressPerTerritoryByIdAndBlockById', async () => {
      const response = await request(app.getHttpServer()).get(`/v1/territories/${territory.id}/blocks/${block.id}`).set('Authorization', `Bearer ${token}`);
      expect(response.status).toBe(400);
    ...
    it('should return 404 for non-existent house in findById', async () => {
      const response = await request(app.getHttpServer()).get('/v1/houses/999999').set('Authorization', `Bearer ${token}`);
    ...
    it('should return 404 for non-existent house in delete', async () => {
      const response = await request(app.getHttpServer()).delete('/v1/houses/999999').set('Authorization', `Bearer ${token}`);
      expect(response.status).toBe(404);
    });
    ...
    it('should return 404 for non-existent street in create', async () => {
      const response = await request(app.getHttpServer()).post('/v1/houses').set('Authorization', `Bearer ${token}`).send({
        number: '200',
        blockId: block.id,
        streetId: 999999,
        territoryId: territory.id,
    ...
    it('should return 404 for non-existent house in update', async () => {
      const response = await request(app.getHttpServer()).put('/v1/houses/999999').set('Authorization', `Bearer ${token}`).send({
        number: '200',
        blockId: block.id,
        streetId: 1,
        territoryId: territory.id,
