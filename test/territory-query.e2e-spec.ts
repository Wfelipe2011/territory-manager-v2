...
describe('Territory Query (findById) e2e', () => {
   ...
   */
  async function seedTerritoryData(args: { tenantId: number; roundNumber: number; socketCount?: number }) {
  ...
  it('deve retornar detalhes do território com contagem correta de conexões e estatísticas', async () => {
    ...
    const response = await request(app.getHttpServer()).get(`/v1/territories/${territory.id}?round=${roundNumber}`).set('Authorization', `Bearer ${token}`);
  ...
  it('deve retornar 404 se o território não existir', async () => {
    ...
    const response = await request(app.getHttpServer()).get(`/v1/territories/999999?round=1`).set('Authorization', `Bearer ${token}`);
  ...
  it('deve isolar dados por rodada (round)', async () => {
    ...
    // Query Round 1
    const res1 = await request(app.getHttpServer()).get(`/v1/territories/${territory.id}?round=1`).set('Authorization', `Bearer ${token}`);
    ...
    // Query Round 2
    const res2 = await request(app.getHttpServer()).get(`/v1/territories/${territory.id}?round=2`).set('Authorization', `Bearer ${token}`);
  ...
  it('deve contar conexões corretamente mesmo com IDs de blocos que são prefixos de outros (ex: 1 e 11)', async () => {
    ...
    await prisma.$executeRawUnsafe(
      `TRUNCATE TABLE "block", "territory", "house", "round", "socket", "territory_block", "territory_overseer", "type", "multi_tenancy", "address", "signature" RESTART IDENTITY CASCADE;`
    );
    ...
    await prisma.round.create({
      data: { roundNumber: 1, territoryId: territory.id, blockId: block1.id, houseId: h1.id, tenantId: tenant.id, completed: false },
    });

    const h11 = await prisma.house.create({
      data: { number: '11', blockId: block11.id, addressId: address.id, territoryId: territory.id, tenantId: tenant.id },
    });
    await prisma.round.create({
      data: { roundNumber: 1, territoryId: territory.id, blockId: block11.id, houseId: h11.id, tenantId: tenant.id, completed: false },
    });
