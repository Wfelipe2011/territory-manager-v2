...
describe('ParametersController (e2e)', () => {
  ...
  describe('/v1/parameters (POST)', () => {
    it('should create a parameter for an admin', async () => {
      ...
      const response = await request(app.getHttpServer()).post('/v1/parameters').set('Authorization', `Bearer ${token}`).send({
        key: 'TEST_KEY',
        value: 'TEST_VALUE',
  ...
  describe('/v1/parameters (GET)', () => {
    it('should list parameters for the tenant', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/v1/parameters').set('Authorization', `Bearer ${token}`);
    ...
    it('should not show parameters from other tenants', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/v1/parameters').set('Authorization', `Bearer ${token2}`);
