...
describe('Password Recovery (e2e)', () => {
  ...
  it.skip('should trigger forgot-password for Wilson', async () => {
    ...
    // Act
    const response = await request(app.getHttpServer()).post('/v1/forgot-password').send({
      email,
    });
  ...
  it('should reset password for Wilson', async () => {
    ...
    // Act
    const response = await request(app.getHttpServer()).post('/v1/reset-password').send({
      token,
    ...
    const loginResponse = await request(app.getHttpServer()).post('/v1/login').send({
      email,
      password: newPassword,
    });
