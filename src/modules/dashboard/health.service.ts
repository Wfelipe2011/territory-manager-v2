...
export class HealthService {
  ...
  async getHealthData() {
    ...
    const { max_connections, countActive, countIdle, sockets, countSignatures } = databaseInfo;
    ...
    return {
      ...
      }] - O servidor está em execução - Produção!`,
  ...
  private async getPgbossInfo(): Promise<{ ready: boolean; queues: Record<string, number> }> {
    const queueNames = ['street.changed', 'USER_JOINED_STREET', 'USER_LEFT_STREET', 'presence.cleanup'];
  ...
  private async getDatabaseInfo() {
    const [[{ max_connections }], [{ count: countActive }], [{ count: countIdle }], sockets, countSignatures] = await Promise.all([
      ...
      this.prismaService.$queryRaw`select count(1) from pg_stat_activity where state = 'active' and datname = ${process.env.POSTGRES_DB}` as Promise<
        { count: bigint }[]
      >,
      this.prismaService.$queryRaw`select count(1) from pg_stat_activity where state = 'idle' and datname = ${process.env.POSTGRES_DB}` as Promise<
        { count: bigint }[]
      >,
  ...
  async getSessionsData(params: { period?: string; groupBy?: string; from?: string; to?: string }): Promise<{
    sessionsGeneral: { slot: string; started: number; active: number }[];
    sessionsByTenant: { slot: string; tenant_name: string; started: number; active: number }[];
  }> {
    ...
    const slotFormat: Intl.DateTimeFormatOptions =
      groupBy === '1d'
