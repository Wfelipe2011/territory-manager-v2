  ...
  async findById(territoryId: number, round: number): Promise<TerritoryOneOutput> {
    ...
    const territory = await this.prisma.$queryRaw<RawTerritoryOne[]>`
      SELECT DISTINCT
        ...
        tov.initial_date,
        tov.finished,
        SUM(CASE WHEN round.completed AND h.number != 'ghost' THEN 1 ELSE 0 END) as positive_completed,
        SUM(CASE WHEN NOT round.completed AND h.number != 'ghost' THEN 1 ELSE 0 END) as negative_completed,
        b.id as block_id,
