...
export class DonationController {
  ...
  async createDonation(@Body() createDonationDto: CreateDonationDto) {
    ...
    const dateFormats = ['DD/MM/YYYY HH:mm:ss', 'DD/MM/YYYY', 'YYYY-MM-DD HH:mm:ss', 'YYYY-MM-DD'];
    ...
    if (!parsedDate) {
      throw new BadRequestException(`Data inválida: ${createDonationDto.payment_date}. Formatos aceitos: DD/MM/YYYY HH:mm:ss, DD/MM/YYYY`);
    }

    // Parse do valor brasileiro (R$11,12 -> 11.12)
    const cleanedPrice = createDonationDto.price.replace(/R\$/g, '').replace(/\./g, '').replace(/,/g, '.').trim();

    const parsedValue = parseFloat(cleanedPrice);

    if (isNaN(parsedValue) || parsedValue <= 0) {
      throw new BadRequestException(`Valor inválido: ${createDonationDto.price}. Use formato brasileiro (ex: R$11,12)`);
    }
