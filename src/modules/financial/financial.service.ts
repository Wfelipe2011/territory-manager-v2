...
export class FinancialService {
  ...
  async create(data: { tenantId: number; value: number; date: Date; description?: string; type: FinancialEntryType; donorName?: string; externalId?: string }) {
  ...
  async getSummary(tenantId?: number) {
    ...
    const totalEntries = entries.filter(e => e.type === FinancialEntryType.POSITIVE).reduce((sum, e) => sum + e.value, 0);

    const totalExits = entries.filter(e => e.type === FinancialEntryType.NEGATIVE).reduce((sum, e) => sum + e.value, 0);
