...
export class AddressBlockService {
  ...
  async manageAddresses(territoryBlockId: number, addresses: AddressDto[] = [], tenantId: number, prisma: PrismaTransaction = this.prisma) {
    ...
    for (const address of renameInputs) {
      ...
      const migratedAddressId = await this.migrateAddressInPlace(linkedAddress.id, linkedAddress.addressId, address.street, territoryBlock, tenantId, prisma);
    ...
    const desiredAddressIds = new Set<number>([...upsertedAddresses.map(a => a.id), ...Array.from(renamedAddressIds)]);
    ...
    if (addressesToDelete.length) {
      ...
      const territoryBlockAddressIds = currentTerritoryBlockAddresses.filter(tba => addressesToDelete.includes(tba.addressId)).map(tba => tba.id);
    ...
    const houses = await prisma.house.findMany({
      where: {
        ...
        OR: addressIds.length
          ? [{ territoryBlockAddressId: { in: territoryBlockAddressIds } }, { territoryBlockAddressId: null, addressId: { in: addressIds } }]
  ...
  async createGhostHouse(
    addressId: number,
    territoryBlock: { blockId: number; territoryId: number },
    territoryBlockAddressId: number,
    tenantId: number,
    prisma: PrismaTransaction
  ) {
  ...
  async syncGhostHouses(territoryId: number, blockId: number, tenantId: number, prisma: PrismaTransaction = this.prisma) {
    ...
    await Promise.all(missingGhostHouses.map(tba => this.createGhostHouse(tba.addressId, tba.territoryBlock, tba.id, tenantId, prisma)));

    // Phase 2: remover ghost houses que agora têm casas reais
    const tbAddressIds = [...new Set(ghostHousesToCleanup.map(g => g.territoryBlockAddressId).filter((id): id is number => id != null))];
    ...
    if (tbAddressIds.length > 0) {
      ...
      orphanGhostIds = ghostHousesToCleanup
        .filter(ghostHouse => {
          ...
          const siblings = allHousesInAddresses.filter(h => h.territoryBlockAddressId === ghostHouse.territoryBlockAddressId);
