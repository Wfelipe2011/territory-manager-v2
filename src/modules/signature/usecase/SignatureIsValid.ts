...
export class SignatureIsValid {
  ...
  async execute(signatureId: string): Promise<void> {
    ...
    if (!signature) throw new NotFoundException('Assinatura inválida');
    if (signature.expirationDate && signature.expirationDate.getTime() < new Date().getTime()) {
      throw new MethodNotAllowedException('Assinatura expirada');
    }
