...
export class TransactionsController {
  ...
  async getBalance() {
    ...
    const totalDonations = entries.filter(e => e.type === 'POSITIVE').reduce((sum, e) => sum + e.value, 0);

    // Mapeamento de custos para manter o contrato original
    const serverCost = entries.filter(e => e.type === 'NEGATIVE' && e.description?.toLowerCase().includes('server')).reduce((sum, e) => sum + e.value, 0);

    const registroBrCost = entries.filter(e => e.type === 'NEGATIVE' && e.description?.toLowerCase().includes('registro')).reduce((sum, e) => sum + e.value, 0);

    const backupCost = entries.filter(e => e.type === 'NEGATIVE' && e.description?.toLowerCase().includes('backup')).reduce((sum, e) => sum + e.value, 0);

    const totalCosts = entries.filter(e => e.type === 'NEGATIVE').reduce((sum, e) => sum + e.value, 0);
