import { ExceptionFilter, Catch, ArgumentsHost, HttpException, HttpStatus, Logger, UnauthorizedException, ForbiddenException, Inject } from '@nestjs/common';
...
export class AllExceptionsFilter implements ExceptionFilter {
  ...
  constructor(@Inject(TraceService) private readonly traceService: TraceService) {}
    ...
    const message = exception instanceof HttpException ? exception.message : 'Erro interno no servidor';

    // Log de erros graves (500) com sessionId
    if (status >= 500) {
      const sessionId = this.traceService.getSessionId() || 'no-session';
      this.logger.error(`[${sessionId}] [${request.method}] ${request.url} - Status: ${status} - Error: ${exception.message || exception}`, exception.stack);
    }
